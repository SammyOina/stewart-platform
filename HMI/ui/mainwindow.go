							...
							g.Condition(pipeline.RecordData,
								g.Layout{
									...
									g.Button("Record Data").Size(120, 100).OnClick(func() {
										...
										pipeline.PitotWriter, _ = fileWriter.NewWriter(timeS+"/"+"pitot.csv", []string{"timestamp", "diffuser", "intake", "test"})
										pipeline.IMUWriter, _ = fileWriter.NewWriter(timeS+"/"+"imu.csv", []string{"timestamp", "yaw", "pitch", "roll"})
										pipeline.StrainWriter, _ = fileWriter.NewWriter(timeS+"/"+"strain_raw.csv", []string{"timestamp", "strain1", "strain2", "strain3", "strain4", "strain5", "strain6"})
										pipeline.ForceMomentsWriter, _ = fileWriter.NewWriter(timeS+"/"+"forces-moments.csv", []string{"timestamp", "Fx", "Fy", "Fz", "Mx", "My", "Mz"})
